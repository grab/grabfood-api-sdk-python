...
class EditOrderItem(BaseModel):
    ...
    EditOrderItem
    """ # noqa: E501
    item_id: StrictStr = Field(description="The item's ID in Grab system that can be obtained from the [Submit Order Webhook](#tag/submit-order-webhook/operation/submit-order-webhook) request payload parameters under `items[].grabItemID`, or `items[].outOfStockInstruction.replacementGrabItemID` for item replacement. External item ID from Partner system is only supported when `ADDED` status and `isExternalItemID: true`.", alias="itemID")
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DELETED', 'UPDATED', 'ADDED', '']):
            raise ValueError("must be one of enum values ('DELETED', 'UPDATED', 'ADDED', '')")
        return value
