...
class Campaign(BaseModel):
    ...
    custom_tag: Optional[Annotated[str, Field(strict=True, max_length=22)]] = Field(default=None, description="Specify the tag for custom bundle offer campaign. Only whitelisted partner is supported as of now.", alias="customTag")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "createdBy", "merchantID", "name", "quotas", "conditions", "discount", "customTag"]

    @field_validator('created_by')
    def created_by_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Grab', 'Merchant', 'Partner']):
            raise ValueError("must be one of enum values ('Grab', 'Merchant', 'Partner')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "quotas": CampaignQuotas.from_dict(obj["quotas"]) if obj.get("quotas") is not None else None,
            "conditions": CampaignConditions.from_dict(obj["conditions"]) if obj.get("conditions") is not None else None,
            "discount": CampaignDiscount.from_dict(obj["discount"]) if obj.get("discount") is not None else None,
