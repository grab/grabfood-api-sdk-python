...
class UpdateSpecialHourResponse(BaseModel):
    """
    ...
    close_immediately: StrictBool = Field(description="Indicate the store status after updating special hours.", alias="closeImmediately")
    ...
    def error_reasons_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['PendingOrders']):
                raise ValueError("each list item must be one of ('PendingOrders')")
        return value
